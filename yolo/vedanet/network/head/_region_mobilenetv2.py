...
class RegionMobilenetv2(nn.Module):
    def __init__(self, num_anchors, num_classes, input_channels=48):
        """ Network initialisation """
        super(RegionMobilenetv2, self).__init__()
        layer_list = [
            # Sequence 2 : input = sequence0
            OrderedDict([
                ('1_convbatch',    vn_layer.Conv2dBatchReLU(96, 24, 1, 1)),
                ('2_reorg',        vn_layer.Reorg(2)),
                ]),

            # Sequence 3 : input = sequence2 + sequence1
            OrderedDict([
                ('3_convbatch',    vn_layer.Conv2dBatchReLU((4*24)+320, 320, 3, 1)),
                ('4_conv',         nn.Conv2d(320, num_anchors*(5+num_classes), 1, 1, 0)),
                ]),
            ]
