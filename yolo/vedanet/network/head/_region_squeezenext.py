...
class RegionSqueezenext(nn.Module):
    def __init__(self, num_anchors, num_classes, input_channels=48):
        """ Network initialisation """
        super(RegionSqueezenext, self).__init__()
        layer_list = [
            # Sequence 2 : input = sequence0
            OrderedDict([
                ('1_convbatch',    vn_layer.Conv2dBatchReLU(128, 16, 1, 1)),
                ('2_reorg',        vn_layer.Reorg(2)),
                ]),

            # Sequence 3 : input = sequence2 + sequence1
            OrderedDict([
                ('3_convbatch',    vn_layer.Conv2dBatchReLU((4*16)+256, 256, 3, 1)),
                ('4_conv',         nn.Conv2d(256, num_anchors*(5+num_classes), 1, 1, 0)),
                ]),
            ]
